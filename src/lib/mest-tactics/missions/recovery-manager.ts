...
export class RecoveryMissionManager {
   ...
   * Keys: Harvest (IC extraction), Sanctuary, Elimination, Bottled
   */
  calculatePredictedScoring(): {
    ...
    // Harvest: VP for extracting ICs (already awarded, so current = predicted for now)
    // This is mission-specific and would need tracking of extracted ICs per side
    // For now, use VP as proxy for harvest success
    ...
    // Sanctuary: +1 VP per turn maintaining 25% BP in sanctuary zone
    // This is tracked per-turn, so we estimate based on current position
    // For simplicity, assume current pattern continues
    for (const side of sideStatuses) {
      // Estimate sanctuary VP based on models in play (simplified)
      ...
      sideScores[side.sideId].keyScores['sanctuary'] = {
        ...
        confidence: sanctuaryVP > 0 ? 0.7 : 0.0, // Moderate confidence as positions can change
