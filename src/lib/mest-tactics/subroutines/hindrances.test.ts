...
describe('calculateHindrancePenalty', () => {
  ...
  it('should return 0 when there are no hindrances', () => {
    const sources: HindranceSources = { woundTokens: 0, fearTokens: 0, delayTokens: 0 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(0);
  });

  it('should return 1 when only woundTokens are present', () => {
    const sources: HindranceSources = { woundTokens: 3, fearTokens: 0, delayTokens: 0 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(1);
  });

  it('should return 1 when only fearTokens are present', () => {
    const sources: HindranceSources = { woundTokens: 0, fearTokens: 2, delayTokens: 0 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(1);
  });

  it('should return 1 when only delayTokens are present', () => {
    const sources: HindranceSources = { woundTokens: 0, fearTokens: 0, delayTokens: 1 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(1);
  });

  it('should return 2 when two types of hindrances are present', () => {
    const sources: HindranceSources = { woundTokens: 2, fearTokens: 1, delayTokens: 0 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(2);
  });

  it('should return 2, ignoring zero-value token types', () => {
    const sources: HindranceSources = { woundTokens: 0, fearTokens: 4, delayTokens: 2 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(2);
  });

  it('should return 3 when all three types of hindrances are present', () => {
    const sources: HindranceSources = { woundTokens: 1, fearTokens: 1, delayTokens: 1 } as any;
  ...
  it('should count hindrance types, not the total number of tokens', () => {
    ...
    const sources: HindranceSources = { woundTokens: 3, fearTokens: 2, delayTokens: 0 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(2);
  });

  it('should accept character-state wound shape (wounds) used by action pipelines', () => {
    const sources: HindranceSources = { wounds: 1, fearTokens: 0, delayTokens: 0 } as any;
    expect(calculateHindrancePenalty(sources)).toBe(1);
  });
