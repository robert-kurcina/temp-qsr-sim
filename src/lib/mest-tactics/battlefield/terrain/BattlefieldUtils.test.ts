...
describe('BattlefieldUtils', () => {
  describe('orientation', () => {
    it('returns 0 for collinear points', () => {
      ...
      const q: Position = { x: 1, y: 1 };
      const r: Position = { x: 2, y: 2 };
  ...
  describe('onSegment', () => {
    ...
    it('returns false when point is not on segment (off-line)', () => {
      const p: Position = { x: 0, y: 0 };
      const q: Position = { x: 1, y: 1 };  // Off the line from (0,0) to (2,0)
      const r: Position = { x: 2, y: 0 };
  ...
  describe('segmentsIntersect', () => {
    it('returns true for intersecting segments', () => {
      ...
      const q1: Position = { x: 2, y: 2 };
      const p2: Position = { x: 0, y: 2 };
      const q2: Position = { x: 2, y: 0 };
      expect(segmentsIntersect(p1, q1, p2, q2)).toBe(true);
    });

    it('returns false for non-intersecting segments', () => {
      const p1: Position = { x: 0, y: 0 };
      const q1: Position = { x: 1, y: 1 };
      const p2: Position = { x: 2, y: 2 };
      const q2: Position = { x: 3, y: 3 };
      expect(segmentsIntersect(p1, q1, p2, q2)).toBe(false);
    });

    it('returns true for collinear overlapping segments', () => {
      const p1: Position = { x: 0, y: 0 };
      const q1: Position = { x: 2, y: 0 };
      const p2: Position = { x: 1, y: 0 };
      const q2: Position = { x: 3, y: 0 };
      expect(segmentsIntersect(p1, q1, p2, q2)).toBe(true);
    });
  });
  ...
  describe('segmentIntersection', () => {
    ...
    it('returns null for parallel segments', () => {
      const p1: Position = { x: 0, y: 0 };
      const q1: Position = { x: 1, y: 1 };
      const p2: Position = { x: 2, y: 0 };
      const q2: Position = { x: 3, y: 1 };
      expect(segmentIntersection(p1, q1, p2, q2)).toBe(null);
    });
  ...
  describe('pointInPolygon', () => {
    ...
    it('returns true for point on edge', () => {
      ...
      const point: Position = { x: 1, y: 0 };
      expect(pointInPolygon(point, polygon)).toBe(true);
    });

    it('returns false for polygon with less than 3 vertices', () => {
      const polygon: Position[] = [{ x: 0, y: 0 }, { x: 1, y: 1 }];
      const point: Position = { x: 0.5, y: 0.5 };
      expect(pointInPolygon(point, polygon)).toBe(false);
    });
  });
  ...
  describe('segmentToSegmentDistance', () => {
    it('returns 0 for intersecting segments', () => {
      const a1: Position = { x: 0, y: 0 };
      const a2: Position = { x: 2, y: 2 };
      const b1: Position = { x: 0, y: 2 };
      const b2: Position = { x: 2, y: 0 };
      expect(segmentToSegmentDistance(a1, a2, b1, b2)).toBe(0);
    });

    it('returns minimum distance for parallel segments', () => {
      const a1: Position = { x: 0, y: 0 };
      const a2: Position = { x: 2, y: 0 };
      const b1: Position = { x: 0, y: 1 };
      const b2: Position = { x: 2, y: 1 };
