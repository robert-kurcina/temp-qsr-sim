...
| **P2 Advanced/Edge Rules** | ⚠️ Indirect audit started; Advanced/Edge remaining |

**Current full-suite baseline:** `npx vitest run` → **148 files, 2381 tests passing**.
**Current coverage baseline:** `npx vitest run --coverage` → **148 files, 2381 tests passing** (`All files`: S 69.85 / B 58.13 / F 75.33 / L 71.24).
...
| `docs/canonical/MEST.Tactics.Advanced-*.txt` | Various | Advanced rules | P2 | ⚠️ Partially Audited (Expanded-Effects pass started) |
...
| Section | File | Status |
|---------|------|--------|
| Advanced Traits | `docs/qsr/08-advanced/08.01-traits.md` | ⚠️ Partially Audited (Expanded-Effects clause map) |
...
- Published module-level P1 coverage metrics with `@vitest/coverage-v8` enabled and validated a full-suite coverage pass (`npx vitest run --coverage`).
